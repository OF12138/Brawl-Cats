...
void GameWindow::paintEvent(QPaintEvent *event)
{
    ...
    for (int i = 0; i < mapWidthInGrids; ++i)
    {
        for (int j = 0; j < mapHeightInGrids; ++j)
        {
            ...
            {
                // 绘制水池图片
                painter.drawPixmap(x, y, poolImage);
            }
            else
            {
                // 不是草丛，显示方格图片
    ...
    for (const auto& enemy : enemies) {
        if (!isInGrass(enemy.position)) { // 新增：只绘制不在草丛内的敌人
            QPixmap currentEnemyImage;
            double enemyDx = playerPosition.x() - enemy.position.x();
            if (enemyDx > 0) {
                currentEnemyImage = enemyImagesRight[enemy.stepCount % 8];
            } else if (enemyDx < 0) {
                currentEnemyImage = enemyImagesLeft[enemy.stepCount % 8];
            } else {
                currentEnemyImage = enemyImagesRight[0];
            }
            int enemyX = static_cast<int>(enemy.position.x()) - offsetX - currentEnemyImage.width() / 2;
            int enemyY = static_cast<int>(enemy.position.y()) - offsetY - currentEnemyImage.height() / 2;
    ...
    for (int i = 0; i < mapWidthInGrids; ++i) {
        for (int j = 0; j < mapHeightInGrids; ++j) {
            ...
            if (isBlock[i][j]) {
                ...
                painter.fillRect(x, y, w, h, Qt::blue); // 新增：绘制水池在小地图上的表示
            } else {
