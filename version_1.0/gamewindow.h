...
public:
    ...
    void stopGameMusic();
    void stopLoseMusic();
    void stopTimers(); // 停止所有定时器
    void startTimers();

signals:
    void gameEnded(); //游戏结束信号
...
private:
    ...
    double playerSpeed;
    int timerId;
    const double minMoveThreshold = 3.0; //最小阈值
    int stepCount;
    ...
    struct Enemy
    {
        ...
        bool isHurt = false; 
        int hurtStartTime = 0;
        bool isFacingRight = true; // 敌人的朝向信息
    };
    std::vector<Enemy> enemies;
    int enemyTimerId;
    const double enemySpeed = 1.0;
    const double enemyStopDistance = 50.0;
    const int hurtDuration = 800; // 受伤持续时间（毫秒）
    void spawnEnemy(int count); // 支持生成指定数量的敌人
    void moveEnemies();
    bool isInGrass(const QPointF& position); // 判断是否在草丛内
    ...
    struct Bullet
    {
        QPointF position;
        QPointF direction;
        int startTime; // 记录子弹发射时间
    };
    std::vector<Bullet> bullets; // 子弹列表
    const double bulletSpeed = 4.0; // 子弹速度，便于修改
    const int bulletLifetime = 2000; // 子弹生命周期（毫秒）
    // 检测子弹与敌人的碰撞
    bool isBulletHittingEnemy(const Bullet& bullet, const Enemy& enemy);

    // 积分相关
    int score = 0; 
    bool gameWon = false; // 游戏是否胜利
    int winStartTime = 0; // 胜利开始时间
    ...
    QHash<QPoint, bool> originalGridStates; // 原始格子状态
    // 技能冷却时间相关
    ...
    // 失败相关
    QPixmap endLoseImage; 
    QMediaPlayer* loseSoundPlayer; 
    QAudioOutput* loseAudioOutput;
    bool gameLost = false; 
    int loseStartTime = 0; // 失败开始时间
