...
void GameWindow::drawMiniMap(QPainter& painter)
{
    ...
    const int margin = 10;          // 小地图左上角和窗口左上角的距离
    ...
    // 创建圆角矩形路径
    QPainterPath path;
    path.addRoundedRect(margin, margin, miniMapWidth, miniMapHeight, borderRadius, borderRadius);
    ...
    for (int i = 0; i < mapWidthInGrids; ++i)
    {
        for (int j = 0; j < mapHeightInGrids; ++j)
        {
            int x = static_cast<int>(i * scale * gridSize + margin + borderWidth);
            int y = static_cast<int>(j * scale * gridSize + margin + borderWidth);
            int w = static_cast<int>((i + 1) * scale * gridSize + margin + borderWidth) - x;
            int h = static_cast<int>((j + 1) * scale * gridSize + margin + borderWidth) - y;
    ...
    painter.drawRoundedRect(margin, margin, miniMapWidth, miniMapHeight, borderRadius, borderRadius);

    // 绘制玩家在小地图上的位置
    int playerMiniX = static_cast<int>(playerPosition.x() * scale + margin + borderWidth);
    int playerMiniY = static_cast<int>(playerPosition.y() * scale + margin + borderWidth);
    ...
    for (const auto& enemy : enemies) {
        if (!isInGrass(enemy.position)) {
            ...
            {
                int enemyMiniX = static_cast<int>(enemy.position.x() * scale + margin + borderWidth);
                int enemyMiniY = static_cast<int>(enemy.position.y() * scale + margin + borderWidth);
...
void GameWindow::resetGame()
{
    ...
    if (!scoreLabel) {
        ...
        scoreLabel->setStyleSheet("background-color: rgba(255, 255, 255, 0.8);"  // 80% 透明度
                                  "color: black;"
                                  "font-size: 20px;"
                                  "font-family: 黑体;"  // 使用黑体
    ...
    // 将玩家位置重置为格子坐标 (7, 7) 的中心
    playerPosition = QPointF(7 * gridSize + gridSize / 2, 7 * gridSize + gridSize / 2);
    targetPosition = playerPosition;
    lastTargetPosition = targetPosition;
